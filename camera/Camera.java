...
public class Camera {
  ...
  ParcelFileDescriptor.AutoCloseInputStream cameraStream;
//  VideoView video;
  ...
  private void prepareMediaRecorder(String outputFilePath) throws IOException {
    ...
    cameraStream = new ParcelFileDescriptor.AutoCloseInputStream(readFD);
    mediaRecorder.setOutputFile(writeFD.getFileDescriptor());
//    mediaRecorder.setPreviewDisplay(pictureImageReader.getSurface());
...
//    mediaRecorder.setOutputFile(outputFilePath);
  ...
  private void createCaptureStreamSession(
          int templateType, String outputFilePath)
    ...
    cameraStream = new ParcelFileDescriptor.AutoCloseInputStream(readFD);
    mediaRecorder.setOutputFile(writeFD.getFileDescriptor());
//    mediaRecorder.setPreviewDisplay(flutterSurface);
...
//    captureRequestBuilder = cameraDevice.createCaptureRequest(templateType);
    ...
    List<Surface> remainingSurfaces = Arrays.asList(mediaRecorder.getSurface());
    if (templateType != CameraDevice.TEMPLATE_PREVIEW) {
      // If it is not preview mode, add all surfaces as targets.
      for (Surface surface : remainingSurfaces) {
        captureRequestBuilder.addTarget(surface);
    ...
    CameraCaptureSession.StateCallback callback =
            new CameraCaptureSession.StateCallback() {
              ...
              public void onConfigured(@NonNull CameraCaptureSession session) {
                ...
                Log.e("CallbackOnConfigured", "Configured");
                ...
                try {
                  ...
                  Log.e("CallbackOnConfigured", "Configured 1");

                  try {

                    Log.e("CallbackOnConfigured", "Configured 1");
                    cameraStream.read(bytes, 0, bufsize);

                    int rc = FFmpeg.execute("-i" +  bytes + "-vsync 1 -f flv \"rtmp://a.rtmp.youtube.com/live2/h75x-88uu-u2ge-847f\"");
...
//      recordingVideo = true;
      createCaptureStreamSession(CameraDevice.TEMPLATE_RECORD, filePath );
